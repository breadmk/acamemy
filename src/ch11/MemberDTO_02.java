...
/*	Object 클래스의 equals 객체 자신과 주어진 객체(obj)를 비교한다. 같으면 true; 다르면 false;
	 여기에서는 회원명과 주민번호가 동일하면 동일객체라고 
	그렇지 않으면 동일객체x
*/
	@Override
	public boolean equals(Object obj) {
//		매개변수 obj의 타입은 Object이므로.. Object를 상속받은 클래스는 다 들어 올 수 있다.
//		따라서 
//		매개변수 obj에 들어온 value가 MemberDTO_02 클래스의 객체이니? 부터 확인.
//		매개변수 obj가 [ MemberDTO_02 클래스의 ] instance 이니?
//		A instanceof B : A가 B의 instance이면 true, 그렇지 않으면 false. 
	...
	/*	 2) 비교객체가 동일한 타입이라면 기준 객체타입으로 강제타입변환 (
	 * 		강제형)변수> MemberDTO_02 member =(MemberDTO_02)obj;
	 */
...
//			2.2. 특정 필드값이 동일한지 비교
			...
			return false;//그렇지 않으면 동일객체 X 이므로 false;
		}
	}
	@Override
	public int hashCode() {
//		name과 ssn값이 같으면 동일한 hashCode가 리턴
		return name.hashCode()+ssn.hashCode();
//		여기에서는 name이 String 이므로 name.hashCode()는 String클래스에서 override 되어진 hashCode()를 이용
	}
	
	/* equals(Object obj) 매소드 재정의시
	 * 1) Object 타입의 매개변수 obj에는 모든 객체가 매개값으로 제공될 수 있으므로
	 * 	  instanceof 연산자를 기준 객체와 동일타입인지 먼저 확인을 해야 한다.
	 * 	    동일타입이면 true를 반환
	 * 
