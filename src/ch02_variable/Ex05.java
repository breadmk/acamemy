...
public class Ex05 {
	...
	public static void main(String[] args) {
		int v=10;
		long v0=10;	//int형 10데이터가 long 타입변수로 자동형변환.
		
		//	long타입변수를 초기화할 때는 정수값 뒤에 소문자l or 대문자L을 붙일 수 있다. 
		//	4byte 정수 데이터가 아니라 8byte 정수 데이터임을 알려준다.
		//	int타입의 저장 범위를 넘어서는 큰 정수에서는 l,L 을 붙여야 한다 -> 우주, 은행권 업무
		long v1=10l;
		long v2=10L;
		...
		char c1 = '가';
		i1=c1;		//	char타입 c1이 int형으로 자동형변환	2byte인 char -> int
		System.out.println("i1 = "+i1);		//i1 = 44032
		
		i1=500;
		long l1 = i1;	//int->long
		System.out.println("l1 = "+l1);		//l1 = 500
		
		i1=200;
		double d1=i1;	//int->double (정수가 실수형태로 자동형변환)
		System.out.println("d1= "+d1);		//d1= 200.0
		...
		//음수10.0을 정수 변수에 할당?
		//int a1 = -10.0; Type mismatch: cannot convert from double to int
		int a2 = (int)-10.0;	//강제형변환
		double a3 = -10.0;		//double 타입값을 double 형 변수에 할당.
		float a4 = (float) -10.0;	//double 타입값을 float타입으로 강제형변환.
		a4 = -10.0f;	//리터럴형변환
