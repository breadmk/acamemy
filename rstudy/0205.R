...
gender_income2<-gender_income%>%group_by(gender2) %>%
...
summary(data$age)  #나이변수 요약
hist(data$age)
age_income=data%>%filter(!is.na(income)) %>%
          group_by(age) %>%
          summarise(avgincome=mean(income))
age_income
...
#연령대별 급여차이
#data$ageg<-ifelse(data$age<30,'초년',ifelse(data$age<60,'중년','노년'))
data<-data %>% mutate(ageg=ifelse(age<30,'초년',
                      ifelse(age<60,'중년','노년')))
mode(data$ageg)
table(data$ageg)
barplot(table(data$ageg))
ageg_income<-data %>% filter(!is.na(income)) %>%
    group_by(ageg) %>%
    summarise(avgincome=mean(income))
ageg_income
#ggplot(data=ageg_income,aes(x=ageg,y=avgincome))+geom_col()
#x축 순서 지정
ggplot(data=ageg_income,aes(x=ageg,y=avgincome))+geom_col()+scale_x_discrete(limits=c('초년','중년','노년'))

#연령대,성별별 급여차이
ageg_gender_income<-data %>% filter(!is.na(income)) %>%
    group_by(ageg,gender2) %>%
    summarise(avgincome=mean(income))
ageg_gender_income
#ggplot(data=ageg_gender_income,aes(x=ageg,y=avgincome))+geom_col()
#성별에 따른 다른색
...
#나이,성별별 급여차이
age_gender_income<-data %>% filter(!is.na(income)) %>%
  group_by(age,gender2) %>%
  summarise(avgincome=mean(income))
...
data %>% filter(!is.na(income)) %>%
  select(code_job,job) %>% head()
job_income<- data %>% filter(!is.na(income)) %>%
    group_by(job) %>%
      summarise(avgincome=mean(income))
job_income  
...
# 성별별 직업빈도
womanjob<-data %>% filter(!is.na(job) & gender2=='여자') %>%
  group_by(job) %>%
  summarise(cnt=n()) %>% 
